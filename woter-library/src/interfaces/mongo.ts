...
export interface IMongoDatabaseServiceContract {
  ...
  isConnected(): boolean;
  createObject<TDocument extends IMongoDocument>(
    collectionName: string,
    document: TDocument
  ): Promise<string>;
  createObjects<TDocument extends IMongoDocument>(
    collectionName: string,
    documents: TDocument[]
  ): Promise<string[]>;
  searchObject<TDocument extends IMongoDocument>(
    collectionName: string,
    filter: IMongoDocument
  ): Promise<TDocument | null>;
  searchObjects<TDocument extends IMongoDocument>(
    collectionName: string,
    filter: IMongoDocument,
  ...
  findOne<TDocument extends IMongoDocument>(
    collectionName: string,
    filter: IMongoDocument
  ): Promise<TDocument | null>;
  findMany<TDocument extends IMongoDocument>(
    collectionName: string,
    filter: IMongoDocument,
    options?: IMongoFindOptions
  ): Promise<TDocument[]>;
  insertOne<TDocument extends IMongoDocument>(
    collectionName: string,
    document: TDocument
  ): Promise<string>;
  updateOne(
